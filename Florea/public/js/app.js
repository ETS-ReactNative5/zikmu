    ...
    if ($(".searchResults").length != 0) {
      if (e.keyCode == 38) {
        if ($(".searchResults.focused").length == 0) {
          ...
          $("#search").attr('action', (_$$0$href = $(".searchResults.focused a")[0].href) !== null && _$$0$href !== void 0 ? _$$0$href : route('search.post'));
          ...
          if ($(".searchResults.focused a")[0] != undefined) {
            ...
            var vali = $("#SearchInput")[0].dataset.value;
            $("#SearchInput").val(vali);
          } else {
            $("#search").attr('action', 'search.php');
            var _vali = $("#SearchInput")[0].dataset.value;
            $("#SearchInput").val(_vali);
          }
        ...
        if ($(".searchResults.focused").length == 0) {
          ...
          $("#search").attr('action', (_$$0$href2 = $(".searchResults.focused a")[0].href) !== null && _$$0$href2 !== void 0 ? _$$0$href2 : 'search.php');
          $("#SearchInput").val($(".searchResults.focused a").text().trim());
        } else {
          ...
          if ($(".searchResults.focused a")[0] != undefined) {
            $("#search").attr('action', $(".searchResults.focused a")[0].href);
            var _vali2 = $("#SearchInput")[0].dataset.value;
            $("#SearchInput").val(_vali2);
          } else {
            $("#search").attr('action', 'search.php');
            var _vali3 = $("#SearchInput")[0].dataset.value;
            $("#SearchInput").val(_vali3);
          }
...
var getData = function getData() {
  ...
  $.ajax({
    method: 'get',
    url: route('api.get_products', {
      q: val
    }),
    data: {},
    success: function success(data) {
      $('#searchResults').html('');
      $("#searchResults").html(data); // $("#searchResults").html('');
      // $.each(data, (i, obj) => {
      //     let $el = axios
...
var trim_special_chars = function trim_special_chars(input) {
  a.forEach(function (p, key) {
...
function str_slug(title, separator) {
  if (typeof separator == 'undefined') separator = '-'; // Convert all dashes/underscores into separator

  var flip = separator == '-' ? '_' : '-';
  title = title.replace(flip, separator); // Remove all characters that are not the separator, letters, numbers, or whitespace.

  title = title.toLowerCase().replace(new RegExp('[^a-z0-9' + separator + '\\s]', 'g'), ''); // Replace all separator characters and whitespace by a single separator
